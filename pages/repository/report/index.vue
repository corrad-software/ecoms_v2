<template>
    ...
    <div>
        ...
        <div class="flex justify-between space-x-2">
            <div class="flex space-x-2 flex-grow">
                ...
                <div class="mt-1 relative">
                  <Button variant="outline" size="sm" @click="isDateRangeModalOpen = true">
                    <Icon name="mdi:calendar" class="mr-2"></Icon>Date Range
                  </Button>

                  <!-- Indicator (Shows only if dateRange has values) -->
                  <span 
                    v-if="dateRange && dateRange.length > 0" 
                    class="absolute top-0 right-0 -mt-1 -mr-1 w-2.5 h-2.5 bg-red-500 rounded-full">
                  </span>
                </div>
      ...
      <div class="overflow-y-auto overflow-x-auto max-h-[660px]">
        <Table>
          <TableHeader>
            <TableRow>
      ...
      <Modal v-model:open="isDateRangeModalOpen">
        ...
        <ModalFooter class="mt-3">
          <Button variant="outline" @click="resetDateRange">Reset</Button>
  ...
  const resetDateRange = () => {
    dateRange.value = []; // Reset date range
  };
  ...
  const filteredRequests = computed(() => {
    ...
    if (dateRange.value && dateRange.value[0] && dateRange.value[1]) {
        ...
        console.log(`${startDate.toISOString()} until ${endDate.toISOString()}`);
